/*
This code is bad, I know this code is bad. It is a project that is meant to be compiled and shown off, not mission critical must be updated for years code.
*/
...
fn print_color(colors: Vec<MinecraftColor>) {
    let overall_color = blocks_to_color(colors.clone());
    for color in colors {
        print!(
            "{}",
            ", ".truecolor(overall_color.r, overall_color.g, overall_color.b)
        );
        match color {
            White => print!(
                "{}",
                "White".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            LightGray => print!(
                "{}",
                "Light Gray".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Gray => print!(
                "{}",
                "Gray".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Black => print!(
                "{}",
                "Black".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Brown => print!(
                "{}",
                "Brown".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Red => print!(
                "{}",
                "Red".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Orange => print!(
                "{}",
                "Orange".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Yellow => print!(
                "{}",
                "Yellow".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Lime => print!(
                "{}",
                "Lime".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Green => print!(
                "{}",
                "Green".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Cyan => print!(
                "{}",
                "Cyan".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            LightBlue => print!(
                "{}",
                "Light Blue".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Blue => print!(
                "{}",
                "Blue".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Purple => print!(
                "{}",
                "Purple".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Magenta => print!(
                "{}",
                "Magenta".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
            Pink => print!(
                "{}",
                "Pink".truecolor(overall_color.r, overall_color.g, overall_color.b)
            ),
        }
    }
    println!();
...
fn lab_distance(first: &RGB<u8>, second: &RGB<u8>, luminance_factor: f64) -> f64 {
    let first = srgb_to_oklab(*first);
    let second = srgb_to_oklab(*second);
    let hyp1: f64 = ((first.a as f64 - second.a as f64).powi(2)
        + (first.b as f64 - second.b as f64).powi(2))
    .sqrt();

    let hyp2: f64 = (hyp1.powi(2)
        + ((first.l as f64 * luminance_factor) - (second.l as f64 * luminance_factor)).powi(2))
    .sqrt();
    hyp2
}
...
fn main() {
    ...
    println!("Luminance (Recommended 0.75):");
    let luminance = user_input_float();
    ...
    println!("Saturation (Recommended 0.25):");
    let saturation = user_input_float();
    ...
    */
    ...
    for wanted_color in beacon_colors {
        ...
        for possible_color in all_block_glass_list.clone() {
            let mut delta_e = lab_distance(&wanted_color.1, &possible_color.0, luminance_factor);
            delta_e = delta_e.abs();
    ...
    for (key, value) in &beacon_data {
        ...
        print!("/tp {} ~ {}", value.x + 0.5, value.y + 0.5);
        //println!("beacon number: {key}");
        //println!("{}, {}", value.x + 0.5, value.y + 0.5);
        //println!("rad: {}", value.rad);
        //println!("delta: {}", value.delta);
        let colors = final_list_of_beacon_glass.get_key_value(&key).unwrap().1;
        print_color(colors.clone());
    }
    ...
    for list_number in 0..total_beacons {
        ...
        let single_de = lab_distance(&color_1_rgb, &color_2_rgb, luminance_factor);
